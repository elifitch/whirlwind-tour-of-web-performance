...
## Debugging
Great, we know all about how the browser makes pixels, but how do we put this to use?  I'm going to walk through two real world ish scenarios and debug two sites' render problems in real time: First, we're going to fix a mobile menu animation that is janky on old phones, and next, we're going to debug a website that has some herky jerky scrolling performance on old and underpowered desktops.
...
In the performance tab we can emulate a slower CPU, which will give us a much clearer picture of what our friend saw.  Lets set it to 20x slowdown.  Believe it or not, mobile devices from just 3 to 4 years ago are actually around 20 times slower than the computer I'm using right now.
[[set it to 20x slower, hit the menu button again]]
noooow we're seeing some definite jank.  Yikes that's really bad.  I wonder how bad it is.  We can open up the chrome framerate monitor in order to test and verify that are changes move things in the right direction.
...
[[inspect element on main]]
Okay so it looks like we're applying the `off-canvas-open` class to the body, which is setting the left property of our main content block to shift it over.  Remember `left` is a layout property, so we're triggering a cascade of reflows but animating the menu in this way.  Lets change it to a composite-only property, transform, to get this done.
...
[[scroll up and down]]
Scrolling around now though, on my nice fast laptop, I don't see any issues.  Lets turn to our friend the CPU throttler, and see if we can get my fast computer to walk a mile in the shoes of a slow computer.
[[20x throttle, scroll around]]
Oof yeah, now we can see this is pretty bad.
[[scroll up to the top and scroll a bit]]
What's interesting is that at the top of the page, the scrolling performance is actually pretty good, no problems here, but once we get into the meat of the page, a little bit, the scrolling performance gets much much worse.  Chrome devtools has a great tool we can turn to to help us visualize where the problems might be.
[[Open up render panel]]
Lets check the box for "paint flashing".  This will flash some green on our display whenever the browser repaints.
[[click it and scroll]]
...
[[inspect the right menu at the root, add will change]]
...
[[toggle on show layer borders]]
See this orange border around the element?  It's held on its own layer now!  Lets do the same with the other menu.
...
[[Point to scrolling performance issues]]
It's worth noting here that we didn't use this checkbox to highlight scrolling performance issues.  Lets refresh and see why.
[[Refresh, toggle it on]]
See how only the left menu here is highlighted as a scroll problem, despite the right menu being complicit as well?  This tool gives you some false positives and omits some real problem areas at times.  It can be a good guidepost, but don't rely on it to do your thinking for you.
