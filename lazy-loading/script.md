...
[CHROME]
Here's our page, a little blog post about some Goats.  There are a handful of images here, and it'll serve as a good case study for any image laden content page.
...
Next we'll make a function to take the an element's data-src attribute and create a normal source attribute for it.  This function would also be the place to add a class to an image to animate or fade it into view, or perform any other onload tasks.
```
...
[lazyload.js]
The function we pass as an argument to the intersection observer constructor by default gets called every time an observed element enters or leaves the viewport.  That means that we need to make a funciton that does a little logic to make sure we load images as they ENTER the viewport, not as they leave.
```
...
Welllllllp that's not good.  Because we're setting our image src attributes in javascript, a user without JS enabled wont see any goats at all!  Luckily there's an extremely straightforward way to fix this.
